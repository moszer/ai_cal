...
try {
  ...
  logger.error('Failed to initialize Passport', { error: error.message });
...
app.post('/api/estimate-calories', protect, upload.single('foodImage'), async (req, res) => {
    ...
    try {
        ...
        const prompt = `
            Analyze the provided food image in detail. You MUST return a JSON object with the following structure:
            1.  "foodItems": An array of objects. Each object in this array MUST contain:
                a.  "name": A string representing the identified food item. Be as specific as possible (e.g., "Grilled Chicken Caesar Salad with Croutons" instead of just "Salad").
                b.  "calories": A string representing the estimated calorie count for that single item. Try to provide a specific number rather than a range (e.g., "270 kcal" rather than "200-300 kcal").
                c.  "proteinGrams": A string representing the estimated protein in grams for that item. Be specific and make an educated guess even if uncertain (e.g., "18g" rather than "N/A").
                d.  "carbsGrams": A string representing the estimated carbohydrates in grams for that item. Be specific even when uncertain (e.g., "22g" rather than "N/A").
                e.  "fatGrams": A string representing the estimated fat in grams for that item. Be specific even when uncertain (e.g., "12g" rather than "N/A").
                f.  "fiberGrams": A string representing the estimated fiber in grams for that item. ALWAYS provide a value - for fruits use 2-4g, for vegetables use 3-6g, for whole grains use 2-5g, for processed foods use 0-2g.
                g.  "sugarGrams": A string representing the estimated sugar in grams for that item. ALWAYS provide a value - for fruits use 10-20g, for desserts use 15-30g, for processed foods check for added sugar (5-15g), for vegetables use 2-5g.
                h.  "sodiumMg": A string representing the estimated sodium in milligrams for that item. ALWAYS provide a value - for processed or restaurant foods use 500-1000mg, for soups use 600-900mg, for home-cooked foods use 200-400mg.
        ...
        try {
            ...
            jsonOutput.totalCalories = jsonOutput.totalCalories || "0";
            jsonOutput.totalProteinGrams = jsonOutput.totalProteinGrams || "0";
            jsonOutput.totalCarbsGrams = jsonOutput.totalCarbsGrams || "0";
            jsonOutput.totalFatGrams = jsonOutput.totalFatGrams || "0";
            jsonOutput.totalFiberGrams = jsonOutput.totalFiberGrams || "0";
            jsonOutput.totalSugarGrams = jsonOutput.totalSugarGrams || "0";
            jsonOutput.totalSodiumMg = jsonOutput.totalSodiumMg || "0";
