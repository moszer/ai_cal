...
import verifyAppleIdToken from '../utils/appleTokenVerifier.js';
...
// @desc    Handle iOS Apple Sign-In
// @route   POST /api/users/auth/apple/ios
// @access  Public
router.post('/auth/apple/ios', async (req, res) => {
  try {
    ...
    logger.info(`iOS Apple Sign-In: ${email} (${appleId})`);
    ...
    if (!user) {
      ...
      const apiKey = uuidv4();
      user = await User.create({
        name: email.split('@')[0], // Use part of email as name since Apple doesn't always provide it
        email,
        appleId,
        apiKey,
        apiCreditsUsed: 0,
      ...
      logger.info(`Created new user via iOS Apple Sign-In: ${email}`);
    } else if (!user.appleId) {
      // User exists but doesn't have Apple ID - link accounts
      user.appleId = appleId;
      if (!user.apiKey) {
        user.apiKey = uuidv4();
      }
      await user.save();
      logger.info(`Linked existing account to Apple: ${email}`);
    }
    ...
    logger.error(`iOS Apple auth error: ${error.message}`);
